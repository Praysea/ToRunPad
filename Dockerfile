...
# Downstream deployments build this Dockerfile directly (RunPod's GitHub
# integration runs a plain `docker build` with no --build-arg), so the default
# has to be "no model": otherwise every rebuild pulls the ~12 GB flux1-dev-fp8
# checkpoint into the image. Models are supplied at runtime instead, either by a
# network volume or by RunPod's Cached models.
ARG MODEL_TYPE=none
